...
{
    public class 労働安全衛生法Repository
    {
        private readonly DocsService docsService;
        private readonly string documentId = "1juUkUUVUAv9AEvbTcJh4vv2fzORSp52AyEHxN43MxY8"; // GoogleドキュメントのID
        private readonly string lawName = "労働安全衛生法";

        public 労働安全衛生法Repository(DocumentConnector docsConnecter)
        {
            docsService = docsConnecter.GetDocsService();
        }
        public void ExportHtml()
        {
            File.WriteAllText(@"C:\drive\work\web\Release\exam\w労働安全衛生法.html", ConvertTextToHtml(GetWholeHtml(), "w労働安全衛生法"));
            File.WriteAllText(@"C:\drive\work\web\Release\exam\s労働安全衛生法.html", ConvertTextToHtml(GetSummaryHtml(), "s労働安全衛生法"));
        }
        ...
        public string GetWholeHtml()
        {
            ...
            foreach (var group in groupedBlocks)
            {
                ...
                {
                    var header = $"<h2>{firstBlock.LawId} {firstBlock.Caption}</h2>";
        ...
        //Googleドキュメントのデータを取得
        private IList<StructuralElement> GetDocumentText()
        {

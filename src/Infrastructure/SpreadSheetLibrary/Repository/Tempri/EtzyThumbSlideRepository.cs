...
{
    public class EtzyThumbSlideRepository
    {
        ...
        private readonly string sheetName = "slide_etzy_thumb";
        public EtzyThumbSlideRepository(SheetConnecter sheetConnecter)
        {
            this.sheetConnecter = sheetConnecter;
            sheetsService = sheetConnecter.GetSheetsService();

        }
        public async Task<SlideEtzyThumbSheetObject> GetSlideEtzySheetObject(int printId)
        {
            ...
            if (value == null)
            {
                return new SlideEtzyThumbSheetObject()
                {
                    PrintId = printId,

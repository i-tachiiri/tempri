...
namespace PrintSiteBuilder.Print.日本語.かけ算
{
    public class 二桁と一桁のかけ算 : IPrint
    {
        ...
        public 二桁と一桁のかけ算()
        {
            PresentationId = "1BQT0EfgH1GG0D4i5PLpwbl3NAuMwOtA4nw2448lqkGI";
            PrintName = "2桁と1桁のかけ算";
            PagesCount = 100;
        ...
        public List<List<string>> GetQuestions1()
        {
            ...
            for (var i = 11; i < 100; i++)
            {
                for (var j = 2; j < 10; j++)
                {
                    ...
                    var question = new List<string> { Number1[0].ToString(), Number1[1].ToString(), "×", " ", Number2[0].ToString() };
                    if (sum.Length == 2)
                    {
                        question.Add(" ");
                    }

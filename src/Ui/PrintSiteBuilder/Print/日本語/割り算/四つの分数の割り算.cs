...
namespace PrintSiteBuilder.Print.日本語.割り算
{
    public class 四つの分数の割り算 : IPrint
    {
        ...
        public 四つの分数の割り算()
        {
            PresentationId = "11gPrzDBzT4oL3wJ6-5JXwbBRpVU6QPyat1PxwruIDL4";
            PrintName = "四つの分数の割り算";
            PagesCount = 100;
        ...
        public List<List<string>> GetQuestions1()
        {
            ...
            while (試行回数 < 1000000 && questions.Count < 5)
            {
                約分回数 = 0;
                試行回数++;
                var pair = AllFractions.OrderBy(x => random.Next()).Take(4).ToList();
                var 分子1 = pair[0][0];
                var 分母1 = pair[0][1];
                var 分子2 = pair[1][0];
                var 分母2 = pair[1][1];
                ...
                if (calcs.Get最大公約数(分子1, 分母4) != 1) 約分回数++;
                if (calcs.Get最大公約数(分子2, 分母1) != 1) 約分回数++;
                if (calcs.Get最大公約数(分子2, 分母3) != 1) 約分回数++;
                if (calcs.Get最大公約数(分子2, 分母4) != 1) 約分回数++;
                if (calcs.Get最大公約数(分子3, 分母1) != 1) 約分回数++;
                if (calcs.Get最大公約数(分子3, 分母2) != 1) 約分回数++;
                if (calcs.Get最大公約数(分子3, 分母4) != 1) 約分回数++;
                if (calcs.Get最大公約数(分子4, 分母1) != 1) 約分回数++;
                if (calcs.Get最大公約数(分子4, 分母2) != 1) 約分回数++;
                if (calcs.Get最大公約数(分子4, 分母3) != 1) 約分回数++;
                if (約分回数 >= 5)
                {
                    ...
                    var 割後分子4 = 計算記号3 == "÷" ? 分母4 : 分子4;
                    var 割後分母4 = 計算記号3 == "÷" ? 分子4 : 分母4;
                    var 最大公約数 = calcs.Get最大公約数(分子1 * 分子2 * 分子3 * 分子4, 分母1 * 分母2 * 分母3 * 分母4);
                    var 回答分子 = 分子1 * 分子2 * 分子3 * 分子4 / 最大公約数;
                    var 回答分母 = 分母1 * 分母2 * 分母3 * 分母4 / 最大公約数;
                    if (回答分子 > 100 || 回答分母 > 100) continue;
                    問題番号++;
